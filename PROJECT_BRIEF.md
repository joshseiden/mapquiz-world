...
- [x] **Learn-mode repeat rework** (spec agreed 2026-08-01, from field feedback after wider sharing — players confused by repeating countries. Diagnosis: misses DO get the review badge in Learn mode as specced, but *tap-won* countries also repeat — unlabeled — because graduation needs 2 tap-wins and the small starter deck cycles fast. Design rule reaffirmed: every repeat must be labeled. Hard mode: NO changes.)
  1. **Graduation threshold lowered 2 → 1:** a single correct tap graduates the country.
  2. **Tap-won countries never return as tap rounds.** Their one return is the graduation typed round, which is **scheduled** — spliced back in 15–25 rounds ahead (the retired two-hint-win window mechanics) rather than waiting for a deck reshuffle — and labeled with the existing promotion pill. Typed win → mastered, benched for the rest of the session (excluded from Learn-mode reshuffles). Typed loss → existing behavior: un-graduated, recycles via miss logic with the review badge.
  3. **Misses unchanged:** 8–15 round recycle + review badge, as shipped.
  4. **Net effect:** every repeat in Learn mode carries a label — review badge ("you missed this one") or promotion pill ("you know this one — type it!"). The unlabeled repeat no longer exists.
  5. Soft-green tap tint now appears after the single tap-win (unchanged visually — the first tap already tinted).
  - **Built 2026-08-01** on `learn-repeats` branch, all 5 points: `endRound()`'s tap-win branch now sets `st.graduated = true` unconditionally on the first win (was gated on `st.tapCorrect >= 2`), and explicitly splices `current` back into `remaining` at `remaining.length - (15 + random(0..10))` — the same pattern the existing miss-recycle splice already uses just below it, reusing that 15–25 window. No other code needed changing: `nextCountry()` already derived `roundKind` from `st.graduated` and already gated the review/promotion badges off `roundKind`/`isReviewRound`, so once graduation is scheduled explicitly instead of left to a reshuffle, the labeling was already correct.
  - **Play-tested locally** via a headless-Chrome CDP harness driving ~220 simulated rounds (always answering tap rounds correctly): confirmed zero cases of a country reappearing as an unlabeled tap round after graduating (the exact bug from the diagnosis), confirmed the promotion badge is visible on every graduation round encountered, and confirmed scheduling lands within the intended window (observed 17 and 21 rounds after the tap win, against a target of 15–25). Verifying this needed temporary debug instrumentation (a `window.__debug()` hook exposing the closures' internal state) added and removed for the test — not part of the shipped diff. One test-harness false alarm along the way, run down and ruled out: an early version of the harness appeared to show a country "stuck" repeating the same graduation round for 30+ rounds — root cause was the test script double-clicking `guessBtn` (once to submit the guess, once more as a blind "next round" click), and on a wrong guess the second click hit `submitGuess()` again with an already-cleared input, silently no-opping; confirmed clean with a properly-paced click sequence (independently re-verified a graduation round resolves correctly on a normal win). Not a product bug — the real 350ms tap-guard on `guessBtn` (unrelated, pre-existing, prevents a fast follow-up tap from skipping the reveal) was the other timing pitfall the harness had to respect.
  - **Play-tested clean by Josh on the deploy preview 2026-08-01** and merged to main; branch deleted.
- [x] **Welcome screen v3** (spec agreed 2026-08-01 — the current single dense paragraph won't get read, and Hard mode deserves a visible entry point at the start):
  1. **Structure replaces the paragraph** — same overlay, new content, top to bottom:
     - Heading: "Welcome to MapQuiz.world" (unchanged)
     - Hook, larger type (~16px, primary text color): "A mystery country lights up on the map. Can you name it?" (two lines)
     - Three short scannable lines, each with a small leading icon/emoji (💡 amber, 🔄 blue, 📈 green — or equivalent inline SVG; keep consistent with the review-badge emoji style):
       · "Hints unlock as you guess"
       · "Miss a country? It comes back until you know it"
       · "Master countries to level up as you play"
  2. **Two start buttons, deliberately unequal, separated by an "or" divider:**
     - Primary (amber, full width): **"Start playing"** → mode = learn (normal play; the word "normal" appears nowhere — the default needs no name)
     - Divider row between the buttons: hairline — "or" (small, muted) — hairline (the login-screen pattern)
     - Secondary (outline, full width, two lines INSIDE the border): **"Start in Hard mode"** + smaller second line "type your answers instead of tapping choices" → mode = test
     - Parallel "Start…" phrasing on both buttons is deliberate — the divider carries the "or".
     - Both save to localStorage exactly as Start does today.
  3. Overlay trigger unchanged (first visit / no saved mode). In-game Hard mode toggle unchanged.
  4. Old intro paragraphs removed entirely — no other copy on the overlay.
  - **Built 2026-08-02** on `welcome-v3` branch, all 4 points: the two `.mode-intro` paragraphs replaced with `.mode-hook` (16px, `var(--text)`, explicit `<br>` between the two sentences rather than relying on fluid wrap, to guarantee the two-line break the spec calls for regardless of viewport width) and a `.mode-features` list (three `<li>`s, each a leading emoji `<span>` + text, 13px muted). Emoji used as-is (💡🔄📈) with no CSS recolor — emoji glyphs render their own native colors regardless of a `color` rule, and the three chosen already read as amber/blue/green natively, matching the spec note. New `.mode-start-hard` secondary button (outline: transparent bg, `var(--accent)` border/text) holds two inner `<span>`s (main line 15px/600, sub line 11px muted) for the two-line-inside-the-border treatment; `.mode-divider` is a flex row with `::before`/`::after` hairlines around the "or" text, the standard login-screen pattern. JS: `modeStartBtn`'s handler and the new `modeStartHardBtn` handler both now call a shared `startWithMode(m)` (mode, saveMode, updateHardModeUI, close, startGame) instead of the old single inline handler — `"learn"` for the primary button, `"test"` for the secondary, both persisting to `localStorage` exactly as before.
  - **Play-tested locally** via a headless-Chrome CDP harness: confirmed both buttons correctly set and persist `mode` to `localStorage`, close the overlay, and start the game in the right state (primary → Learn mode, tap-answer UI; secondary → Hard mode ON, typed-guess UI) — screenshotted the overlay itself for a visual check against the spec (heading, hook, three feature lines, primary/divider/secondary stack) and both post-start states.
  - **Play-tested clean by Josh on the deploy preview 2026-08-02** and merged to main; branch deleted.
