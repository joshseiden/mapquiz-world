...
- [ ] **Viewport-aware layout** (spec agreed 2026-07-24 — answer buttons cut off at the bottom of the screen in Learn mode; this is the structural fix, and the core of the icebox "mobile layout pass"):
  ...
  - **Built 2026-07-28** (per spec above, all 4 points): `body` is now a flex column (`100dvh` min-height) with `header` fixed and `.layout` as `flex:1`. `.main` (map + controls) is a nested flex column; `.map-wrap` is `flex:1; min-height:320px` and the `<svg>` now fills it (`width/height:100%`, default `preserveAspectRatio` letterboxes rather than distorting); `.controls` is `flex-shrink:0` so it's never squeezed. `.sidebar` stretches to match column height (`align-items:stretch` on `.layout`, the flex default, now set explicitly) and scrolls independently (`overflow-y:auto`) when its reveal content runs long — same principle applies when the existing `@media (max-width:900px)` breakpoint stacks `.layout` into a column, since `.main`'s `flex:1` there fills available vertical space ahead of the sidebar without any extra rule needed. Compaction pass: `.answer-btn` padding 16px→13px (~46px measured height, still clear of the 44px floor), tighter gaps on `.answer-grid`/`.learn-question`/`.guesses`/`.feedback`/`.layout`. **Not yet play-tested** — no headless-browser tooling available in this environment (same limitation noted for the feedback form on 2026-07-18); Josh is play-testing directly on the deploy preview against the test matrix in point 4.
